...
namespace RandomizerCommon
{
    ...
    public class Util
    {
        ...
        public static HashSet<T> IntersectAll<T>(IEnumerable<HashSet<T>> sets)
        {
            HashSet<T> ret = null;
            foreach (HashSet<T> set in sets)
            {
                ...
                {
                    ret.IntersectWith(set);
                }
            }
            if (ret == null) throw new ArgumentException("Internal error: cannot intersect no sets");
            return ret;
        }

        public static HashSet<T> UnionAll<T>(IEnumerable<HashSet<T>> sets)
        {
            HashSet<T> ret = null;
            foreach (HashSet<T> set in sets)
            {
        ...
        public static int AddCount<K>(IDictionary<K, int> dict, K key, int count = 1)
        {
            if (dict.TryGetValue(key, out int exist))
            {
                exist += count;
            }
            else
        ...
        public static int AddCountNest<K, V, T>(IDictionary<K, T> dict, K key, V value, int count = 1)
            where T : IDictionary<V, int>, new()
        {
            if (!dict.ContainsKey(key)) dict[key] = new T();
            return AddCount(dict[key], value, count);
        }

        public static T PopDeranged<T>(IList<T> items, T original)
        {
            T replace = items[items.Count - 1];
            if (Equals(replace, original) && items.Count >= 2)
            {
                // Take the second-to-last instead of last to prefer derangements
                replace = items[items.Count - 2];
        ...
        public abstract class Taggable
        {
            ...
            // Do this at the level of the text, and the list should update after.
            // This is stateful so it's only meant for producing/loading config.
            // The list also doesn't prevent duplicates so use HasTag before this anyway.
        ...
        public static uint XxStringHash(string s)
        {
            byte[] bytes = Encoding.UTF8.GetBytes(s);
            byte[] result = XxHash32.Hash(bytes);
            return BitConverter.ToUInt32(result, 0);
        }

        // id generator: avoid vowels and 0 1
        private static readonly List<char> SeedLetters = "bcdfhjklmnpqrstvwxz23456789".ToList();
        public static string GenerateStringSeed(Random random = null)
        {
            random ??= new Random();
            // log(2^32)/log(27) = ~6.73
            char[] str = new char[7];
            for (int i = 0; i < str.Length; i++)
            {
                str[i] = Choice(random, SeedLetters);
            }
            return new(str);
        }
        ...
        public static void Shuffle<T>(Random random, IList<T> list, int start, int count)
        {
            // Fisher Yates shuffle - O(n)
            if (start + count > list.Count) throw new Exception($"Internal error: bad shuffle with start {start} and count {count} with length {list.Count}");
            for (var i = start; i < start + count - 1; i++)
            {
                int j = random.Next(i, start + count);
                T tmp = list[i];
                list[i] = list[j];
                list[j] = tmp;
            }
        ...
        {
            if (n == 1) return new List<T> { Choice(random, list) };
            // Partial shuffle is more efficient, but it's basically fine
            List<T> ret = list.ToList();
            Shuffle(random, ret);
            if (ret.Count > n)
            {
                ret.RemoveRange(n, ret.Count - n);
            }
            return ret;
        }
        ...
        public static void TestStringSeeds()
        {
            HashSet<uint> intSeeds = new();
            HashSet<uint> strSeeds = new();
            Random x = new Random();
            for (int i = 0; i < 100_000_000; i++)
            {
                string seedStr = GenerateStringSeed(x);
                uint seed = XxStringHash(seedStr);
                strSeeds.Add(seed);
                intSeeds.Add((uint)x.Next());
            }
            Console.WriteLine($"int {intSeeds.Count}, str {strSeeds.Count}");
        }
        ...
        public static uint FNV1AHash(uint val)
        {
            uint hash = 2166136261;
            foreach (byte input in BitConverter.GetBytes(val))
            {
                hash ^= input;
                hash *= 16777619;
            }
        ...
        public static Color BlendColors(Color c1, Color c2, double per)
        {
            per = per * per;
            ...
            return Color.FromArgb(red, grn, blw);
        }
