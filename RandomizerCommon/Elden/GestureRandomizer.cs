...
namespace RandomizerCommon
{
    public class GestureRandomizer
    {
        ...
        public void Randomize(RandomizerOptions opt)
        {
            ...
            // Erudition is partly optional if horse parkour is allowed (for Liurnia, not Bellum).
            // Default starting gestures are useful for multiplayer (seamless coop) but it shouldn't be a huge loss to change them.
            bool debug = false;

            // This is param ids, not good ids
            HashSet<int> norandom = new HashSet<int>
            {
                // Finger Snap
                73,
                // Erudition
                103,
                // The Ring 2, handled below
                109,
                // May the Best Win
                111,
                // O Mother
                115,
                // Miquella's Ring 2, handled below
                116,
                // These result in bugged NPC invaders who use the gestures and can get stuck (TODO: try to set flag in emevd to avoid it?)
                // Reverential Bow
                4,
                // Casual Greeting
                8,
            ...
            Dictionary<int, PARAM.Row> gestureGoods = game.Params["EquipParamGoods"].Rows
                .Where(r => (r.ID >= 9000 && r.ID < 9100) || (opt["dlc"] && r.ID >= 2009000 && r.ID < 2009100))
            ...
            foreach (PARAM.Row row in game.Params["GestureParam"].Rows)
            {
                ...
                // Console.WriteLine($"{row.ID}: {good} {name}");
            }
            List<int> targetGestures = sourceGestures.ToList();
            Random gestureRandom = new Random((int)opt.Seed + 14);
            int dlcIndex = targetGestures.FindIndex(id => id >= 111);
            if (opt["dlc"] && opt["dlcsilo"] && dlcIndex >= 0)
            {
                // Relies on sourceGestures being a SortedSet, as well as param ordering
                Shuffle(gestureRandom, targetGestures, 0, dlcIndex);
                Shuffle(gestureRandom, targetGestures, dlcIndex, targetGestures.Count - dlcIndex);
            }
            else
            {
                Shuffle(gestureRandom, targetGestures);
            }
            int targetIndex = 0;
            // From source to destination gesture
            Dictionary<int, int> mapping = new Dictionary<int, int>();
            bool silent = opt["silent"];
            if (!silent) Console.WriteLine("-- Gesture placements");
            foreach (int source in sourceGestures)
            {
                int target = targetGestures[targetIndex++];
                mapping[source] = target;
                if (!silent) Console.WriteLine($"Replacing {gestureNames[target]}: {gestureNames[source]}");
            }
            if (!silent) Console.WriteLine();
            if (mapping.TryGetValue(108, out int ringDest)) mapping[109] = ringDest;
            if (mapping.TryGetValue(113, out int mringDest)) mapping[116] = mringDest;
