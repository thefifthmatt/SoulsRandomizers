...
namespace RandomizerCommon
{
    public class HintMarker
    {
        ...
        public class HintArea
        {
            ...
            // Defeat flag. Normally filled in from itemlocations.txt, but should be specified otherwise (usually needed for overworld bosses)
            public uint BossFlag { get; set; }
            ...
            [YamlIgnore]
            public Entity BossId { get; set; }
            [YamlIgnore]
            public uint BossMarker { get; set; }
            [YamlIgnore]
        ...
        public class HintItem
        {
            ...
            // First name of multiple, if it can optionally be a multi-item
            public string MultiName { get; set; }
            // Better name for more complicated items
            public string DisplayName { get; set; }
            // Excuses
            public string Comment { get; set; }
            // A way to check the item's area has actually been explored. Not used
        ...
        public void Write(RandomizerOptions opt, Permutation perm, PermutationWriter.Result permResult)
        {
            ...
            {
                if (!game.BonfireCats.TryGetValue(row.ID, out string name))
                {
            ...
            {
                if (!game.BonfireNames.TryGetValue(row.ID, out string name))
                {
                    int nameId = (int)row["textId1"].Value;
                    name = placeFmg[nameId];
                }
                if (string.IsNullOrWhiteSpace(name)) continue;
            ...
            Dictionary<int, uint> allMarkerFlags = new();
            void addConditionalMark(int placeNameId, string mapId, Vector3 pos, uint appearFlag, uint disappearFlag, bool ignore = false)
            {
                ...
                row["eventFlagId"].Value = ignore ? 6001u : appearFlag;
                row["textEnableFlagId3"].Value = ignore ? 6001u : appearFlag;
                row["textDisableFlagId3"].Value = ignore && ignoreDisappear ? 0u : disappearFlag;
                if (appearFlag != 6001)
                {
                    allMarkerFlags[row.ID] = appearFlag;
                }
                // Aboveground, underground
                bool dlc = game.IsEldenDlcMap(mapId);
                row["dispMask00"].Value = (byte)(dlc ? 0 : 1);
                row["dispMask01"].Value = (byte)(dlc ? 0 : 1);
                row["dispMask02"].Value = (byte)(dlc ? 1 : 0);
            ...
            if (opt["markbonfires"])
            {
                ...
                foreach (PARAM.Row row in game.Params["BonfireWarpParam"].Rows)
                {
                    ...
                    (Vector3 pos, EldenCoordinator.Tile tile) = coord.ToGlobalCoords(mapParts, mapPos);
                    List<byte> toMap = new List<byte> { (byte)tile.Type, (byte)tile.X, (byte)tile.Z, 0 };
            ...
            foreach (HintItem item in config.Items)
            {
                ...
                IEnumerable<string> getLogicItems(string itemName)
                {
                    if (ann.Items.ContainsKey(itemName))
                    {
                        return new[] { itemName };
                    }
                    // MultiName is used to given items multinames explicitly, but this can be done for most key items now, so try to infer it
                    else if (ann.Items.TryGetValue(itemName + "1", out ItemKey multiKey)
                        && ann.MultiItems.TryGetValue(multiKey, out MultiItem multiItem))
                    {
                        return multiItem.Names;
                    }
                    else throw new Exception($"Internal error: missing {itemName}");
                }
                foreach (string itemName in item.Name.Split(' ').SelectMany(getLogicItems))
                {
                    items[itemName] = item;
                }
            }
            HashSet<ItemKey> hintItemKeys = new HashSet<ItemKey>(items.Select(e => ann.Items[e.Key]));
            HashSet<ItemKey> excludedItems = new();
            if (!(opt["dlc"] && ann.KeyItemSilo == AreaSiloType.None))
            {
                // Only hint DLC items if base game key item chains can end up there.
                // Otherwise, may need a way to silo requested hints.
                excludedItems.UnionWith(hintItemKeys.Where(i => game.IsEldenDlcItem(i)));
                hintItemKeys.RemoveWhere(i => game.IsEldenDlcItem(i));
            }
            List<ItemKey> missingItems = ann.ItemGroups["keyitems"].Where(i => !hintItemKeys.Contains(i) && !excludedItems.Contains(i)).ToList();
            ...
            Dictionary<string, HintArea> areas = new();
            HashSet<string> unusedAreas = new();
            Dictionary<uint, Entity> bossIds = new();
            Dictionary<string, Entity> bossEvents = new();
            foreach (HintArea area in config.Areas)
            {
                ...
                if (area.Boss != null)
                {
                    if (uint.TryParse(area.Boss, out uint bossId))
                    {
                        bossIds[bossId] = null;
                    }
                    else
            ...
            foreach (HintArea area in config.Areas)
            {
                if (area.Parent == null || area.Unused) continue;
            ...
            // TODO: DLC filtering for hint areas. !ann.Areas[a].HasTag("dlc")
            List<string> missingAreas = ann.Areas.Keys.Where(a => !areas.ContainsKey(a) && !unusedAreas.Contains(a)).ToList();
            ...
            foreach (HintArea area in config.Areas)
            {
                if (area.Boss == null || area.Unused) continue;
                Entity id = uint.TryParse(area.Boss, out uint bossId) ? bossIds[bossId] : bossEvents[area.Boss];
                if (id == null) throw new Exception($"[{area.Includes}]: entity {area.Boss} not found");
                area.BossId = id;
                if (area.BossFlag <= 0)
                {
                    if (!data.BossFlags.TryGetValue(id.EntityID, out uint bossFlag))
                    {
            ...
            Dictionary<ItemKey, List<ItemLocKey>> itemSlots = new Dictionary<ItemKey, List<ItemLocKey>>();
            Dictionary<ItemLocKey, string> logOrder = new Dictionary<ItemLocKey, string>(); 
            Dictionary<ItemLocKey, string> slotAreas = new Dictionary<ItemLocKey, string>();
            Dictionary<ItemLocKey, Entity> slotLocations = new Dictionary<ItemLocKey, Entity>();

            foreach (KeyValuePair<ItemLocKey, List<ItemLocKey>> entry in perm.GetMapping(Permutation.RandomSilo.Finite))
            {
                ItemLocKey target = entry.Key;
                foreach (ItemLocKey source in entry.Value)
                {
                    if (!hintItemKeys.Contains(source.Item) && !permResult.SlotEventFlags.ContainsKey(source)) continue;
                    ItemLocation itemLoc = data.GetItemLoc(target);
            ...
            foreach (KeyValuePair<ItemKey, List<ItemLocKey>> entry in itemSlots)
            {
                entry.Value.Sort((a, b) => logOrder[a].CompareTo(logOrder[b]));
            }

            // After the main marker flag range, these appear to be unused
            uint startMarkerFlag = 78700;
            uint markerFlag = startMarkerFlag;
            // Map from marker flag to the range of flags it is exclusive with. The range is [start, end)
            Dictionary<uint, (uint, uint)> markerFlagClearRange = new Dictionary<uint, (uint, uint)>();
            ...
            Dictionary<ItemType, string> itemFmgName = new Dictionary<ItemType, string>
            {
                ...
                [ItemType.Protector] = "ProtectorName",
                [ItemType.Accessory] = "AccessoryName",
                [ItemType.Goods] = "GoodsName",
            ...
            int getItemInAreaId(ItemKey item, string text, bool vague)
            {
                ...
                // Previously used hintItemLabelArea if vague, but it's just way too long
                messages.SetFMGEntry(game, FMGCategory.Item, "PlaceName", newId, hintItemLabel, itemName, areaName);
                return newId;
            }
            ...
            void placeBonfireMarker(HintArea area, int placeNameId, uint appearFlag, uint disappearFlag, bool ignore, string slotName)
            {
            ...
            // TODO DLC: Do the config
            bool ignoreMissing = false;

            // Item markers by item, with the item EntityId and item get flag
            Dictionary<string, uint> itemMarkers = new();
            // Area markers by item, with the item's area's bonfire and item get flag
            Dictionary<string, uint> itemAreaMarkers = new();
            // Like boss markers but with the item disappear flag
            Dictionary<string, uint> itemAreaBossMarkers = new();
            foreach (KeyValuePair<string, HintItem> entry in items)
            {
                string item = entry.Key;
                ItemKey key = ann.Items[item];
                if (!itemSlots.TryGetValue(key, out List<ItemLocKey> slots)) continue;
                if (!slotAreas.TryGetValue(slots[0], out string place)) continue;
                if (!permResult.ItemEventFlags.TryGetValue(key, out int signedItemFlag)) continue;
                // PermutationWriter (and much of LocationData, except BossFlags) doesn't use unsigned flags currently
                uint itemFlag = (uint)signedItemFlag;
                if (ignoreMissing && !areas.ContainsKey(place)) continue;
                ...
                if (slotLocations.TryGetValue(slots[0], out Entity id) && id.Position is Vector3 pos)
                {
                    addConditionalMark(
            ...
            for (uint i = startMarkerFlag; i < markerFlag; i++)
            {
                markerFlagClearRange[i] = (startMarkerFlag, markerFlag);
            }

            // Very similar, but for multiple items in slots
            Dictionary<ItemLocKey, uint> slotMarkers = new();
            Dictionary<ItemLocKey, uint> slotAreaMarkers = new();
            foreach (KeyValuePair<string, ItemKey> entry in upgradeItems)
            {
                ...
                getMenuItemId(key);
                if (!itemSlots.TryGetValue(key, out List<ItemLocKey> slots)) continue;
                uint slotMarkerFlag = markerFlag;
                foreach (ItemLocKey slot in slots)
                {
                    if (!slotAreas.TryGetValue(slot, out string place)) continue;
                    if (!permResult.SlotEventFlags.TryGetValue(slot, out int signedItemFlag)) continue;
                    uint itemFlag = (uint)signedItemFlag;
                    if (ignoreMissing && !areas.ContainsKey(place)) continue;
                    HintArea area = areas[place];
                    if (area.MainBonfireId > 0)
                    ...
                    if (slotLocations.TryGetValue(slot, out Entity id) && id.Position is Vector3 pos)
                    {
                ...
                for (uint i = slotMarkerFlag; i < markerFlag; i++)
                {
            ...
            int getPriority(string item, int deps)
            {
                ...
                if (!itemSlots.TryGetValue(ann.Items[item], out List<ItemLocKey> slots)
                    || !slotLocations.TryGetValue(slots[0], out Entity id)
                    || string.IsNullOrEmpty(id.MapName)) return 80 + deps;
                if (miniRe.IsMatch(id.MapName) || id.MapName.StartsWith("m34")) return 60 + deps;
                if (id.MapName.StartsWith("m6")) return 40 + deps;
                return 40 + deps;
            }
            ...
            // TODO: Handle multi-items properly, looking at ReqCount
            string itemFromConfigEntry(string item) => ann.Items.ContainsKey(item) ? item : item + "1";
            ...
            {
                item = itemFromConfigEntry(item);
                int calcFlag(int flag, string dep) => flag > 0 ? flag : permResult.ItemEventFlags[ann.Items[dep == "self" ? item : itemFromConfigEntry(dep)]];
                ItemKey key = ann.Items[item];
            ...
            {
                ItemKey key = ann.Items[item];
                if (!itemSlots.TryGetValue(key, out List<ItemLocKey> slots))
                {
                    return (null, null);
                }
                slotAreas.TryGetValue(slots[0], out string area);
                itemMarkers.TryGetValue(item, out uint itemMarker);
                itemAreaMarkers.TryGetValue(item, out uint itemAreaMarker);
                ...
                uint mainMarker = itemMarker == 0 ? itemAreaMarker : itemMarker;
                (uint startClearFlag, uint endClearFlag) = markerFlagClearRange[mainMarker];
                if (itemMarker == 0) itemMarker = itemAreaMarker;
                if (itemAreaMarker == 0) itemAreaMarker = itemMarker;
                uint bossMarker = hintArea.BossMarker;
                uint bossFlag = hintArea.BossFlag;
                if (itemAreaBossMarkers.TryGetValue(item, out uint itemBossMarker))
                {
                    bossMarker = itemBossMarker;
                }
                object[] machineArgs = new uint[]
                {
                    itemMarker, itemAreaMarker, bossMarker, bossFlag,
                    startClearFlag, endClearFlag - startClearFlag
                }.Select(i => (object)(int)i).ToArray();
            ...
            // This has an extra level of nesting to show the menu
            {
                ...
                foreach (HintUpgradeItem upgrade in config.UpgradeItems)
                {
                    ...
                    List<(ItemLocKey, int)> keys = upgrade.Keys
                        .SelectMany(key => itemSlots.TryGetValue(key, out List<ItemLocKey> keys) ? keys : new List<ItemLocKey>())
                    ...
                    foreach ((ItemLocKey key, int itemFlag) in keys)
                    {
                        ...
                        // (item marker, area marker, boss marker, boss flag, start clear flag, clear flag count)
                        slotMarkers.TryGetValue(key, out uint itemMarker);
                        slotAreaMarkers.TryGetValue(key, out uint itemAreaMarker);
                        uint mainMarker = itemMarker == 0 ? itemAreaMarker : itemMarker;
                        (uint startClearFlag, uint endClearFlag) = markerFlagClearRange[mainMarker];
                        if (itemMarker == 0) itemMarker = itemAreaMarker;
                        if (itemAreaMarker == 0) itemAreaMarker = itemMarker;
                        object[] machineArgs = new uint[] { itemMarker, itemAreaMarker, 0, 0, startClearFlag, endClearFlag - startClearFlag }.Select(i => (object)(int)i).ToArray();
            ...
            */
            ...
            if (allMarkerFlags.Count > 0)
            {
                ...
                foreach (KeyValuePair<int, uint> entry in allMarkerFlags)
                {
        ...
        internal void DumpMinidungeonMarkers()
        {
            ...
            Dictionary<string, SortedSet<uint>> mapBosses = new();
            ...
            foreach (AreaAnnotation annArea in ann.Areas.Values)
            {
                ...
                HintArea mark = new HintArea
                {
                    ...
                    MainBonfire = bonfire,
                    Parent = req,
                    Boss = mapBosses.TryGetValue(annArea.Maps, out SortedSet<uint> ids) ? string.Join(" ", ids) : "?????",
                };
                areas.Add(mark);
            }
