...
namespace RandomizerCommon
{
    public class SkillSplitter
    {
        ...
        public Assignment SplitAll()
        {
            ...
            FMGX itemName = game.ItemFMGs["アイテム名"];
            FMGX itemDesc = game.ItemFMGs["アイテム説明"];
            FMGX weaponName = game.ItemFMGs["武器名"];
            FMGX weaponDesc = game.ItemFMGs["武器説明"];
            ...
            foreach (PARAM.Row r in game.Params["SkillParam"].Rows)
            {
                ...
                int text = (sbyte)r["menuPageNo"].Value;
                if (!texts.ContainsKey(text)) continue;

                int descItem = (int)r["virtualWeaponId"].Value;
                ...
                ushort iconId = (ushort)weaponRow["iconId"].Value;

                int good = baseId++;
                PARAM.Row newGood = game.AddRow("EquipParamGoods", good);
                GameEditor.CopyRow(baseGood, newGood);
                ItemKey goodKey = new ItemKey(ItemType.Goods, good);
                ...
                // But pop up on acquisition (previously Unk20 = 6)
                newGood["isDrop"].Value = (byte)0;
                newGood["isNoticeLog"].Value = (byte)1;
                newGood["isNoticeDialog"].Value = (byte)1;
            ...
            {
                row["skillPoint"].Value = (uint)row["skillPoint"].Value / 5;
            }
            foreach (PARAM.Row row in game.Params["GameAreaParam"].Rows)
            {
                row["bonusSkillPoint"].Value = (uint)row["bonusSkillPoint"].Value / 5;
            }
