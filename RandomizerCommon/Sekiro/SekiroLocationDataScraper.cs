...
namespace RandomizerCommon
{
    public class SekiroLocationDataScraper
    {
        ...
        public SekiroLocationDataScraper(GameData game)
        {
            this.game = game;
            logUnused = false;
            addUnused = false;
        ...
        public LocationData FindItems()
        {
            ...
            SortedDictionary<int, List<Entity>> usedItemLots = allLocs.usedItemLots;
            SortedDictionary<int, List<Entity>> usedBaseShops = allLocs.usedBaseShops;
            ...
            foreach (KeyValuePair<int, List<Entity>> entry in usedItemLots)
            {
                ...
                while (itemLots[itemLot] != null)
                {
                    ...
                    for (int i = 1; i <= 8; i++)
                    {
                        ...
                        if (id != 0)
                        {
                            uint type = (uint)(int)row[$"lotItemCategory0{i}"].Value;
                            int points = (ushort)row[$"lotItemBasePoint0{i}"].Value;
                            int quantity = (ushort)row[$"lotItemNum0{i}"].Value;
                            ...
                            if (itemRow != null && item.Type == ItemType.Goods && (byte)itemRow["goodsType"].Value == 7)
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    // This is only used in Elden Ring, so keep it int for now
                                ...
                                {
                                    scope = new ItemScope(ScopeType.Entity, (int)entities.Select(e => e.EntityID).Min());
                                }
                                else
                                {
                            ...
                            Location location = new Location(LocationType.Lot, itemLot, itemText, entities, quantity, chance, baseLocation);
                    ...
                    if (simple)
                    {
                        SortedSet<string> areas = new SortedSet<string>(entities.Select(e => game.LocationNames[e.MapName]));
                        SortedSet<string> models = new SortedSet<string>(entities.Select(e => e.Name.StartsWith("o") ? "Treasure" : game.EntityName(e)));
                        text2 = $"{string.Join(", ", models)}: {string.Join(", ", areas)}";
                        if (models.All(x => x == "unknown")) { text2 = "Unused/Unknown"; }
                        if (models.All(x => x == "Unused NPC")) { text2 = "Unused NPC"; }
                        else if (models.Any(x => x == "Unused NPC")) { models.Remove("Unused NPC"); if (areas.Count > 1) areas.Remove("Global"); text2 = $"{string.Join(", ", models)}: {string.Join(", ", areas)}"; }
                    }
                    else
                    {
                    ...
                    if ((byte)row["isProgressOffset"].Value == 1)
                    {
                        ...
                        for (offset = curOffset + 10; offset <= 50; offset += 10)
                        {
                            ...
                            if (offRow != null && (byte)offRow["isProgressOffset"].Value == 1)
                            {
                                break;
                            }
                        }
            ...
            Dictionary<int, Location> baseShops = new Dictionary<int, Location>();
            foreach (PARAM.Row row in shops.Rows)
            {
                ...
                int qwc = (int)row["flagId_forRelease"].Value;
                ...
                float priceRate = (float)row["value_Magnification"].Value;
                ...
                if (material != -1)
                {
                    ...
                    costText = $" for {materialQuant} {game.Name(new ItemKey(ItemType.Goods, materialItem))}";
                }
                if (value != 0 || costText == "")
                {
                    ...
                    if (actualCost == -1)
                    {
                        actualCost = (int)game.Item(item)["sellValue"].Value;
                    }
                ...
                Location location = new Location(
                    ...
                    usedBaseShops.ContainsKey(baseShop) ? usedBaseShops[baseShop] : new List<Entity>(),
                    quantity,
                    1,
            ...
            foreach (KeyValuePair<ItemKey, Dictionary<ItemScope, List<Location>>> entry in locations)
            {
                ...
                foreach (KeyValuePair<ItemScope, List<Location>> entry2 in entry.Value)
                {
                    ...
                    SortedSet<int> shopQwcs = new SortedSet<int>(locKeys.Where(k => k.Type == LocationType.Shop).Select(k => (int)shops[k.ID]["flagId_forRelease"].Value)
                    ...
                    LocationScope locationScope = new LocationScope(SoulsIds.GameSpec.FromGame.SDT, scope.Type, id, allShop, modelBase, onlyShops);
                    data.AddLocations(entry.Key, scope, locationScope, locKeys);
                }
                if (data.Items.TryGetValue(entry.Key, out ItemLocations itemLocs))
                {
                    itemLocs.Unique = unique > 0;
                }
            }
        ...
        private ItemLocs FindItemLocs()
        {
            ...
            {
                usedItemLots = new SortedDictionary<int, List<Entity>>(),
                usedBaseShops = new SortedDictionary<int, List<Entity>>(),
                baseLotsToCreate = new Dictionary<int, int>(),
                // TODO: See if we can have a reasaonable processing step
                newEntityLots = new Dictionary<uint, int>(),
            };
            ...
            foreach (KeyValuePair<string, MSBS> entry in game.SekiroMaps)
            {
                ...
                foreach (MSBS.Entry obj in entry.Value.Parts.GetEntries())
                {
                    ...
                    Entity id;
                    int esdId = 0;
                    List<uint> groupIDs = part.EntityGroupIDs.Where(groupID => groupID > 0).Select(GameData.Convert).ToList();
                    if (part is MSBS.Part.Enemy enemy)
                    {
                        esdId = partEsds.ContainsKey(enemy.Name) ? partEsds[enemy.Name] : -1;
                        id = new Entity(location, enemy.Name, GameData.Convert(enemy.EntityID), enemy.NPCParamID, enemy.CharaInitID, groupIDs);
                    }
                    else if (part is MSBS.Part.Object || logEntities)
                    {
                        id = new Entity(location, part.Name, GameData.Convert(part.EntityID), GroupIDs: groupIDs);
                    }
                    else
                    {
                        continue;
            ...
            if (logEntities)
            {
                foreach (KeyValuePair<uint, List<Entity>> entry in usedEntities.OrderBy(e => e.Key))
                {
                    Console.WriteLine($"{entry.Key}: {string.Join(", ", entry.Value.Select(e => $"{game.EntityName(e, true)} in {e.MapName}"))}");
                }
            }
            List<Entity> unusedEsd = new List<Entity> { new Entity("", "Unknown Dialogue") };
            ...
            foreach (PARAM.Row row in npcs.Rows)
            {
                ...
                PARAM.Cell cell = row["itemLotId_1"];
                ...
                if (!usedNpcs.ContainsKey(npcID))
                {
                    ...
                    if (addUnused)
                    {
                        AddMulti(ret.usedItemLots, itemLot, new Entity("", "Unused NPC"));
                    }
                    continue;
                }
            ...
            foreach (KeyValuePair<uint, int> entry in entityItemLots)
            {
                ...
                else
                {
                    ...
                    id = new List<Entity> { new Entity("", "from entity") };
                }
            ...
            foreach (KeyValuePair<uint, int> entry in treasureCarpDrops)
            {
                uint entityId = entry.Key;
            ...
            foreach (KeyValuePair<int, string> entry in scriptLots)
            {
                ...
                AddMulti(ret.usedItemLots, itemLot, new Entity(entry.Value, "Scripted"));
            }
            foreach (int itemLot in talkLots)
            {
                ...
                AddMulti(ret.usedItemLots, itemLot, new Entity("", "from talk"));
            }
            int lastLot = 0;
            ...
            foreach (PARAM.Row lot in itemLots.Rows)
            {
                ...
                bool connected = (byte)lot["isProgressOffset"].Value == 1;
                ...
                else
                {
                    if (!ret.usedItemLots.ContainsKey(itemLot))
                    {
                        ...
                        if (addUnused)
                        {
                            AddMulti(ret.usedItemLots, itemLot, new Entity("", "unknown"));
                        }
                    }
                }
                lastLot = itemLot;
        ...
        private static readonly Dictionary<uint, int> treasureCarpDrops = new()
        {
            {1000450,13200000},
            {1000451,13200000},
            {1000452,13200000},
        ...
        private static readonly Dictionary<uint, List<int>> extraEntityScriptDrops = new Dictionary<uint, int>()
        {
        ...
        private static readonly Dictionary<uint, List<int>> additionalEntityItemLots =
            extraEntityScriptDrops
                .Union(new Dictionary<uint, List<int>>
                {
