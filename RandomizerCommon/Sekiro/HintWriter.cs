        ...
        public void Write(RandomizerOptions opt, Permutation permutation)
        {
            ...
            if (opt["writehints"])
            {
                ...
                foreach (KeyValuePair<ItemKey, string> entry in game.Names())
                {
                    ...
                    if (!(key.Type == ItemType.Goods && (key.ID == 9020 || key.ID >= 9200 && key.ID <= 9228))) continue;
            ...
            foreach (ItemKey key in allItems)
            {
                ...
                ItemLocKey targetKey = permutation.GetFiniteTargetKey(lookup);
                ItemLocation itemLoc = data.GetItemLoc(targetKey);
