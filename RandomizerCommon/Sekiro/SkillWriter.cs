    ...
    public class SkillWriter
    {
        ...
        public void RandomizeTrees(Random random, Permutation permutation, SkillSplitter.Assignment split)
        {
            ...
            foreach (PARAM.Row r in param.Rows)
            {
                ...
                {
                    ID = (int)r.ID,
                    Item = (int)r["virtualWeaponId"].Value,
                    Equip = (int)r["acquireWeaponId"].Value,
                    Flag = (int)r["unlockEventFlag"].Value,
                    Placeholder = (int)r["unenforementWepId"].Value,
                    SpEffects = new[] { (int)r["spEffect1"].Value, (int)r["spEffect2"].Value, (int)r["spEffect3"].Value },
                    EmblemChange = (byte)r["addResourceItemA"].Value != 0,
                };
                ...
                {
                    ID = (int)r.ID,
                    Col = (short)r["dispCol"].Value,
                    Row = (short)r["dispRow"].Value,
                    Text = data.ID < 400 && texts.TryGetValue((sbyte)r["menuPageNo"].Value, out ItemKey text) ? text : null,
                };
            ...
            {
                r["virtualWeaponId"].Value = data.Item;
                r["unlockEventFlag"].Value = data.Flag;
                r["acquireWeaponId"].Value = data.Equip;
                r["spEffect1"].Value = data.SpEffects[0];
                r["spEffect2"].Value = data.SpEffects[1];
                r["spEffect3"].Value = data.SpEffects[2];
                r["unenforementWepId"].Value = data.Placeholder;
                r["addResourceItemA"].Value = (byte)(data.EmblemChange ? 1 : 0);
            }
            ...
            if (split == null)
            {
                ...
                Dictionary<ItemKey, string> textLocations = texts.Values.ToDictionary(t => t, t => {
                    ItemLocKey target = permutation.GetFiniteTargetKey(t);
                    textWeight[t] = permutation.GetLogOrder(target);
                    SlotAnnotation sn = ann.Slot(data.GetItemLoc(target).LocScope);
            ...
            for (int i = 0; i < 10; i++)
            {
                ItemKey baseMat = new ItemKey(ItemType.Goods, 9700 + i * 10);
                ...
                for (int j = 1; j < 10; j++)
                {
                    ...
                    if (names.ContainsKey(wep))
                    {
                        ...
                        if (mat != null)
                        {
                            ...
                            {
                                specialMats[wep] = new ItemKey(ItemType.Goods, extraMat);
