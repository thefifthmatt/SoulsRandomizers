...
namespace RandomizerCommon
{
    public class EventConfig
    {
        ...
        public class EventSpec : AbstractEventSpec
        {
            ...
            // TODO: Split out into separate file for Sekiro
            public List<ItemEventConfig.ItemFlagTemplate> ItemTemplate { get; set; }
        ...
        public class ScriptSpec
        {
            ...
            public List<ScriptReplaceCommand> Replaces { get; set; }
        }

        public class ScriptReplaceCommand
        {
        ...
        public class EnemyTemplate
        {
            ...
            // Edit for changing what triggers gravelorded maps.
            // Format is: start <flag> / end <flag> / manage
            public string Gravelord { get; set; }
            ...
            public uint Invincibility { get; set; }
            // Conditional edit for removing time checks for night bosses
            ...
            // The main format is like "<type> <entity> <initial> <wakeup>", 0 if not provided in this event, and arg refs acceptable.
            // The current types:
            // pose <entity> <initial> <wakeup>
            // wakeup <entity> <wakeup>
            // gravity <entity> <initial> <wakeup> <other common func> - ignored by default unless enemy defines a primary arena, replaced with event with same args
            // gravityarg <entity> <initial> <wakeup> <gravity arg> - gravity, but if enemy has "drop" tag, sets gravity arg true and clears animations
            // (ensure they have 4080/4085 if needed)
            // Edits event itself:
            // active <entity> - removes checks for standby speffects before the main condition
            // passive <entity> - active, plus disables/enables AI (for X0_4) if the animation is -1 (for X4_4)
            // suspend <entity> <initial> - active, plus disables/enables AI if animation is -1 and there's no param-based AI check
            // suspendgravity <entity> <initial> - suspend, plus adds maphit/gravity condition and 4080/4085
            ...
            // What to do with regions if a chr command, <replace> <type> <ids>
            // <replace> can be event or arg here. In enemy config, can be global or copy
            // - chrpoint (exact), arenapoint (center/random), arenabox10 (random), arena (bgm), arenasfx (center), or dist10.
            ...
            // Initialized events to rewrite, for chr events initializing other chr events.
            // The caller events and callee inits are stored separately and matched per (entity, caller, callee, slot).
            public string SubInit { get; set; }
            ...
            public bool IsDefault() =>
                ...
                (Entity == 0 && DefeatFlag == 0 && AppearFlag == 0 && StartFlag == 0 && MusicFlag == null
                    ...
                    && TreeDragons == null && TreeDragonFlags == null && Animation == null && Gravelord == null);
        ...
        public class CommandSegment
        {
            ...
            // presetup: Before first/second setups, some commands. Should leave the boss disabled and inactive, for cases like Margit.
            // firstsetup: After presetup, sets up the boss to appear dynamically at some point
            // secondsetup: After presetup, finishes enabling the boss
            ...
            // Whether to add invincibility commands to this location, to enable (in setup) and disable (in start). DS3 only
            public bool Invincibility { get; set; }
            // Whether to duplicate all commands in this segment for helpers tagged cmdhelper.
            // Caveat: this currently only replaces the main entity id, not any regions/helpers, just for basic enable commands.
            public bool AddHelpers { get; set; }
        ...
        public class Dupe
        {
            ...
            // For MAIN commands, it's <command name> <and/or>
        ...
        public record SubInitKey(uint Source, uint Target, EventKey Caller, EventKey Callee, int Slot);
    }
}
