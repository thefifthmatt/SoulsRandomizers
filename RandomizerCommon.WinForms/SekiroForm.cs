...
namespace RandomizerCommon
{
    public partial class SekiroForm : Form
    {
        ...
        private void UpdateLabels()
        {
            ...
            string loc;
            if (weights.SpreadRatio > 0.86) loc = $"Much better rewards for difficult and late{unfairText} locations.";
            else if (weights.SpreadRatio > 0.55) loc = $"Better rewards for difficult and late{unfairText} locations.";
            else if (weights.SpreadRatio > 0.3) loc = $"Slightly better rewards for difficult and late{unfairText} locations.";
            else if (weights.SpreadRatio > 0.001) loc = "Most locations for items are equally likely. Often results in a lot of early memories and prayer beads.";
            ...
            if (!options["norandom"])
            {
                ...
                else if (weights.KeyItemChain <= 6) chain = "Key items will usually be in different areas and depend on each other.";
                // else if (weights.KeyItemChain <= 10) chain = "Key items will usually form long chains across different areas.";
        ...
        private async void randomize_Click(object sender, EventArgs e)
        {
            ...
            await Task.Factory.StartNew(() => {
                ...
                try
                {
                    string gameExe = Path.GetFullPath(@"..\sekiro.exe");
                    randomizer.Randomize(rand, FromGame.SDT, status => { SetStatus(status); }, enemyPreset: selectedPreset, gameExe: gameExe);
