    ...
    public class CharacterWriter
    {
        ...
        public void Write(Random random, RandomizerOptions options)
        {
            ...
            foreach (ItemKey key in data.Data.Keys)
            {
                ...
                {
                    PARAM64.Row magic = magics[key.ID];
                    if (magic != null)
                    {
            ...
            PARAM64 chara = game.Param("CharaInitParam");
            // Just for testing ;)
            bool cheat = false;
            for (int i = 0; i < 10; i++)
            {
                PARAM64.Row row = chara[startId + i];
                ...
                if (cheat)
                {
                    PARAM64 reinforce = game.Param("ReinforceParamWeapon");
                    ...
                    foreach (string wep in weaponSlots)
                    {
                        ...
                        {
                            id = id - (id % 100);
                            PARAM64.Row item = game.Item(new ItemKey(ItemType.WEAPON, id));
            ...
            Func<ItemType, PARAM64.Cell, float> cellWeight = (type, cell) =>
            {
            ...
            foreach (PARAM64.Row row in chara.Rows.Where(r => r.ID > startId + 10))
            {
